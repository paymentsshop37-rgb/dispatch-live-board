import { calculateReportSummary, selectReportTransactions } from "../reporting/reportSummary.js";
import { reportCsv } from "../reporting/summaryRenderers.js";
...
const reports = [
  ...
  ["technician-payments-due", "Technician Payments Due"], ["technician-payment-history", "Technician Payment History"], ["profitability", "Profitability Report"], ["dispatcher-profit", "Jobs & Profit by Dispatcher"], ["red-internal-control", "Red Internal Control Queue"],
...
export default function AccountingExportCenter({ model, outstanding, settings, session, rangeLabel, onNotice }) {
  ...
  async function prepare(id) {
    if (id === "dispatcher-profit") return { model, settings, invoicePayments: [], techTransactions: [] };
    ...
    return { model: { ...model, outstandingSentInvoices: outstanding }, settings, pendingTechJobs: model.techDueJobs, invoicePayments: selectReportTransactions(invoicePayments, model.jobs), techTransactions: selectReportTransactions(techTransactions, model.jobs) };
  }
  async function run(id, label, format) {
    const key = `${id}-${format}`; setBusy(key);
    try {
      const payload = await prepare(id);
      ...
      if (format === "xlsx") {
        ...
        if (format === "csv") download(new Blob([reportCsv(data.headers, data.rows, calculateReportSummary(data.summaryRows, { kind: data.summaryKind }))], { type: "text/csv;charset=utf-8" }), filename(label, "csv"));
        else {
          const { createAccountingPdf } = await import("./accountingPdf.js");
          const blob = id === "dispatcher-profit"
            ? (await import("./dispatcherProfitPdf.js")).createDispatcherProfitPdf({ jobs: payload.model.jobs, ...options, footer: settings?.pdf_footer })
            : createAccountingPdf({ title: label, ...data, model: id === "executive-accounting" ? payload.model : null, generatedBy: options.generatedBy, filterLabel: rangeLabel, footer: settings?.pdf_footer || "Confidential - NTTR" });
...
function reportTable(id, payload) { if (id === "dispatcher-profit") { const report = buildDispatcherProfitReport(payload.model.jobs); const stats = dispatcherStatistics(payload.model.jobs); return { summaryRows: payload.model.jobs, headers: [...report.summaryHeaders, "Completed Jobs", "Completion Rate (%)", "Cancelled Jobs", "Cancellation Rate (%)"], rows: report.summary.map((row,i) => { const p = [...stats.people, stats.total][i]; return [...row, p.completed, Number((p.completionRate*100).toFixed(1)), p.cancelled, Number((p.cancellationRate*100).toFixed(1))]; }) }; } const jobs = id === "completed-jobs" ? payload.model.jobs.filter(isCompleted) : id === "cancelled-jobs" ? payload.model.jobs.filter(isCancelled) : id === "dry-runs" ? payload.model.jobs.filter(isDryRun) : id === "red-internal-control" ? payload.model.redJobs : id === "technician-payments-due" ? payload.pendingTechJobs : payload.model.jobs; if (id === "accounts-receivable") return { summaryRows: payload.model.receivables, headers: ["Invoice #", "Due Date", "Company", "Reference #", "Total Bill", "Amount Paid", "Balance Due", "Aging", "Status"], rows: payload.model.receivables.map((row) => [row.invoiceNumber, row.dueDateLabel, row.company, row.referenceNumber, row.totalBill, row.amountPaid, row.balanceDue, row.agingBucket, row.paymentStatus]) }; if (id === "invoice-payment-history") return { summaryRows: payload.invoicePayments, summaryKind: "transactions", headers: ["Payment Date", "Invoice #", "Amount", "Method", "Confirmation", "Status"], rows: payload.invoicePayments.map((row) => [row.payment_date, row.invoice_number, row.amount, row.payment_method, row.confirmation_number, row.voided_at ? "Voided" : "Active"]) }; if (id === "technician-payment-history") return { summaryRows: payload.techTransactions, summaryKind: "transactions", headers: ["Payment Date", "Technician", "Amount", "Method", "Confirmation", "Status"], rows: payload.techTransactions.map((row) => [row.payment_date, row.technician_name, row.amount, row.payment_method, row.confirmation_number, row.voided_at ? "Voided" : "Active"]) }; return { summaryRows: jobs, headers: ["Date", "Invoice #", "Reference #", "Company", "Technician", "Dispatcher", "Status", "Invoice Status", "Tech Payment", "Total Bill", "Parts", "Tech Labor", "Estimated Profit"], rows: jobs.map((job) => [job.date, job.invoiceNumber, job.referenceNumber, job.company, job.technician, job.dispatcher, job.status, job.invoiceStatus, job.techPaymentStatus, job.totalBill, job.parts, job.techLabor, estimatedProfit(job)]) }; }
