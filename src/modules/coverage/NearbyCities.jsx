...
export default function NearbyCities({ aliases, isAdmin, busy, onRemove }) {
  ...
  return <div className="mt-4"><div className="flex items-center justify-between gap-2"><strong>Nearby Cities ({aliases.length})</strong><button type="button" aria-expanded={open} onClick={() => setOpen(!open)} className="min-h-11 rounded-xl bg-slate-100 px-3 font-bold">{open ? "Hide Cities" : "View Cities"}</button></div>
