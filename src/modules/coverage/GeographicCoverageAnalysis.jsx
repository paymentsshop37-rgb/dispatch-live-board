        ...
        <div className="relative aspect-[1.7/1] min-h-[360px] w-full bg-[radial-gradient(circle_at_50%_45%,rgba(37,99,235,0.13),transparent_60%)]">
          ...
          {filtered.map((row) => {
            ...
            return <React.Fragment key={row.id}>
              {showRadius && <button type="button" aria-label={`${row.area_name} coverage radius`} onClick={() => setSelected(row)} className="absolute -translate-x-1/2 -translate-y-1/2 rounded-full border opacity-30" style={{ left: `${point.x}%`, top: `${point.y}%`, width: `${Math.max(24, SERVICE_AREA_RADIUS_MILES * 0.55)}px`, height: `${Math.max(24, SERVICE_AREA_RADIUS_MILES * 0.55)}px`, borderColor: tone, backgroundColor: `${tone}22` }} />}
