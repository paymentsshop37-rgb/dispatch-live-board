        ...
        <div className="relative aspect-[1.7/1] min-h-[360px] w-full bg-[radial-gradient(circle_at_50%_45%,rgba(37,99,235,0.13),transparent_60%)]">
          ...
          {filtered.map((row) => {
            ...
            return <React.Fragment key={row.id}>
              {showRadius && <button type="button" aria-label={`${row.area_name} coverage radius`} onClick={() => setSelected(row)} className="absolute -translate-x-1/2 -translate-y-1/2 rounded-full border opacity-30" style={{ left: `${point.x}%`, top: `${point.y}%`, width: `${Math.max(24, Number(row.coverage_radius_miles) * 0.55)}px`, height: `${Math.max(24, Number(row.coverage_radius_miles) * 0.55)}px`, borderColor: tone, backgroundColor: `${tone}22` }} />}
...
function money(value) { return new Intl.NumberFormat("en-US", { style: "currency", currency: "USD", minimumFractionDigits: 2, maximumFractionDigits: 2 }).format(value || 0); }
function csv(value) { return `"${String(value ?? "").replace(/"/g, '""')}"`; }
function exportAreaCsv(rows) { const headers = ["Service Area", "Primary City", "State", ...statusColumns.map(([, label]) => label), "Exact Cities Included", "Active Technicians", "Last Job", "Days Since Last Job", "Percentage of All Jobs", "Change vs Previous Period"]; const data = rows.map((row) => [row.area_name, row.primary_city, row.state, ...statusColumns.map(([key]) => row[key]), row.exactCities.join("; "), row.activeTechnicians.length, row.lastJobDate, row.daysSinceLastJob ?? "", row.percentage.toFixed(2), row.change.toFixed(2)]); download(reportCsv(headers, data, calculateReportSummary(reportJobsFromGroups(rows), { includeFinancial: false })), "service-area-analysis.csv", "text/csv;charset=utf-8"); }
function exportUnassigned(jobs) { const headers = ["Job ID", "Invoice", "Exact City", "State", "Latitude", "Longitude", "Coordinates Available", "Closest Service Area", "Distance Miles"]; const data = jobs.map((job) => [job.id, job.invoiceNumber, job.city, job.state, job.latitude ?? "", job.longitude ?? "", Number.isFinite(job.latitude) && Number.isFinite(job.longitude) ? "Yes" : "No", job.areaAssignment.closestArea?.area_name || "", job.areaAssignment.closestDistance?.toFixed(2) || ""]); download(reportCsv(headers, data, calculateReportSummary(jobs, { includeFinancial: false })), "unassigned-coverage-jobs.csv", "text/csv;charset=utf-8"); }
function printAreas(rows) { const popup = window.open("", "_blank"); if (!popup) return; popup.document.write(`<html><head><title>Coverage Analysis</title><style>body{font-family:Arial;padding:24px}table{border-collapse:collapse;width:100%;font-size:10px}th,td{border:1px solid #ccc;padding:5px;text-align:right}th:first-child,td:first-child{text-align:left}</style></head><body><h1>NTTR Geographic Demand and Coverage Analysis</h1><table><tr>${["Service Area", ...statusColumns.map(([, label]) => label), "Active Techs", "Last Job"].map((value) => `<th>${value}</th>`).join("")}</tr>${rows.map((row) => `<tr><td>${row.area_name}</td>${statusColumns.map(([key]) => `<td>${row[key]}</td>`).join("")}<td>${row.activeTechnicians?.length || 0}</td><td>${row.lastJobDate || "Never"}</td></tr>`).join("")}</table>${summaryHtml(calculateReportSummary(reportJobsFromGroups(rows), { includeFinancial: false }))}<script>window.print()</script></body></html>`); popup.document.close(); }
