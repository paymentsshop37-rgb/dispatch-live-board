...
export function getJobTimestamp(job) {
  ...
  const timeOnly = normalizeJobTime(job?.time || job?.job_time);
...
export function compareJobsChronologically(a, b, sortOrder = "newest") {
  const direction = sortOrder === "oldest" ? -1 : 1;
  const timestampDifference = getJobTimestamp(b) - getJobTimestamp(a);
  if (timestampDifference !== 0) return timestampDifference * direction;
  const createdDifference = String(b?.createdAt || b?.created_at || "").localeCompare(String(a?.createdAt || a?.created_at || ""));
  if (createdDifference !== 0) return createdDifference * direction;
  return String(b?.id || "").localeCompare(String(a?.id || "")) * direction;
}
