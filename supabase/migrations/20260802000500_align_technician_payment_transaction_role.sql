-- Keep transaction entry authorization aligned with the shared operational
-- Tech Payment rule. Accounting Center routing remains separately admin-only.
...
begin
  ...
  if not public.can_update_technician_payment() then
